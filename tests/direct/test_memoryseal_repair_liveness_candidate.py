...
def _deploy(
    ...
    contract = direct_deploy(
        "contracts/memoryseal_liveness_candidate.py"
    )
...
def _create_liveness_repair_child(
    direct_vm,
    contract,
    owner,
    body,
...
def test_canceled_repair_child_allows_parent_cancel(
    direct_vm,
    direct_deploy,
):
    contract, owner = _deploy(
    ...
    (
        ...
        _,
        child_id,
    ) = _create_liveness_repair_child(
        direct_vm,
        contract,
        owner,
        "Corrected evidence before child cancellation.",
    )
    ...
    outsider = create_address(
        "memoryseal-terminal-child-outsider"
    )
    ...
    assert parent.repair_evidence_id != ""


def test_rejected_repair_child_allows_parent_cancel(
    direct_vm,
    direct_deploy,
):
    contract, owner = _deploy(
    ...
    (
        ...
        contract,
        owner,
        "Corrected evidence before semantic rejection.",
    )
...
def test_expired_repair_child_allows_parent_expiry(
    direct_vm,
    direct_deploy,
):
    ...
    (
        ...
        contract,
        owner,
        "Corrected evidence before deadline expiry.",
    )
    ...
    outsider = create_address(
        "memoryseal-parent-expiry-after-terminal-child"
    )
...
def test_repair_required_child_blocks_parent_and_can_continue(
    direct_vm,
    direct_deploy,
):
    contract, owner = _deploy(
    ...
    (
        ...
        contract,
        owner,
        "Corrected evidence that will be tampered during review.",
    )
    ...
    assert child.state == "REPAIR_REQUIRED"
    ...
    replacement_v3 = _register_repair_version(
        direct_vm,
        contract,
        policy_id,
        replacement,
        3,
        "Third version repairs the failed child evidence.",
    )

    grandchild_set = sorted(
        [
            replacement_v3["evidence_id"],
            other["evidence_id"],
        ]
    )
    ...
    assert (
        grandchild.repairs_claim_id
        == child_id
    )

    assert (
        int(grandchild.review_deadline)
        <= int(child.review_deadline)
    )
...
def test_supported_repair_child_keeps_parent_immutable(
    direct_vm,
    direct_deploy,
):
    from datetime import datetime, timezone
    ...
    (
        ...
        contract,
        owner,
        "Corrected evidence for successful repair.",
    )
    ...
    outsider = create_address(
        "memoryseal-supported-child-expiry-outsider"
    )
    ...
    assert final_parent.state == "REPAIR_REQUIRED"
    assert (
        final_parent.repair_child_claim_id
        == child_id
    )
