...
class MemorySeal(gl.Contract):
    ...
    def _repair_child_blocks_parent_closure(self, claim: Claim) -> bool:
        child_id = claim.repair_child_claim_id
        if child_id == '':
            return False
        child = self._require_claim(child_id)
        return child.state not in (CLAIM_REJECTED, CLAIM_CANCELED, CLAIM_EXPIRED)
    ...
    def cancel_claim(self, claim_id: str) -> None:
        ...
        if self._repair_child_blocks_parent_closure(claim):
            raise gl.vm.UserError('REPAIR_CHILD_ALREADY_EXISTS')
        now = _now_seconds()
