    ...
    for character in value:
        if character not in allowed:
            raise gl.vm.UserError(
                f"{error_prefix}_INVALID_CHARACTER"
            )
...
def _validate_source_url(
    ...
    origin_path = source_url.startswith(
        publisher_origin + "/"
    )

    origin_query = source_url.startswith(
        publisher_origin + "?"
    )

    origin_fragment = source_url.startswith(
        publisher_origin + "#"
    )
...
class MemorySeal(gl.Contract):
    ...
    def _get_effective_subject_head(
        self,
        subject_id: str,
        now: int,
    ) -> str:
        head_id = self.subject_heads.get(
            subject_id,
            "",
        )
        ...
        if head.subject_id != subject_id:
            raise gl.vm.UserError(
                "SUBJECT_HEAD_SUBJECT_MISMATCH"
            )

        if head.state != CLAIM_SUPPORTED:
            raise gl.vm.UserError(
                "SUBJECT_HEAD_STATE_INVALID"
            )
    ...
    def _append_subject_history(
        self,
        subject_id: str,
        claim_id: str,
    ) -> None:
        count = int(
            self.subject_history_counts.get(
                subject_id,
                u64(0),
            )
        )

        recorded_head = self.subject_heads.get(
            subject_id,
            "",
        )
        ...
        if count == 0:
            if recorded_head != "":
                raise gl.vm.UserError(
                    "SUBJECT_HISTORY_HEAD_MISMATCH"
                )
        else:
            previous_key = _pair_key(
                subject_id,
                str(count - 1),
            )
            ...
            if (
                previous_key
                not in self.subject_history_claim_ids
            ):
                raise gl.vm.UserError(
                    "SUBJECT_HISTORY_ENTRY_NOT_FOUND"
                )

            previous_claim_id = (
                self.subject_history_claim_ids[
                    previous_key
                ]
            )
        ...
        self.subject_history_counts[
            subject_id
        ] = u64(count + 1)

    @gl.public.view
    def get_recorded_subject_head(
        self,
        subject_id: str,
    ) -> str:
        _require_token(
        ...
        return self.subject_heads.get(
            subject_id,
            "",
        )

    @gl.public.view
    def get_subject_head(
        self,
        subject_id: str,
    ) -> str:
        _require_token(
        ...
        return self._get_effective_subject_head(
            subject_id,
            now,
        )

    @gl.public.view
    def get_subject_history_count(
        self,
        subject_id: str,
    ) -> u64:
        _require_token(
        ...
        return self.subject_history_counts.get(
            subject_id,
            u64(0),
        )

    @gl.public.view
    def get_subject_history_claim_id(
        self,
        subject_id: str,
        index: u64,
    ) -> str:
        ...
        return self.subject_history_claim_ids[
            key
        ]
    ...
    def is_policy_issuer(
        ...
        return self.policy_issuer_allowed.get(
            key,
            False,
        )

    @gl.public.view
    def is_policy_origin(
        self,
        policy_id: str,
        origin: str,
    ) -> bool:
        ...
        return self.policy_origin_allowed.get(
            key,
            False,
        )
    ...
    @gl.public.view
    def get_latest_evidence_version(
        self,
        policy_id: str,
        stable_record_id: str,
    ) -> u64:
        ...
        return self.latest_evidence_versions.get(
            lineage_key,
            u64(0),
        )
    ...
    def create_policy(
        ...
        if min_issuers > MAX_POLICY_ISSUERS:
            raise gl.vm.UserError(
                "TOO_MANY_REQUIRED_ISSUERS"
            )

        if min_origins > MAX_POLICY_ORIGINS:
            raise gl.vm.UserError(
                "TOO_MANY_REQUIRED_ORIGINS"
            )

        if min_records < 2:
            raise gl.vm.UserError(
                "MIN_EVIDENCE_BELOW_TWO"
            )

        if (
            min_records < min_issuers
            or min_records < min_origins
        ):
            raise gl.vm.UserError(
        ...
        if (
            ...
            or max_records > MAX_EVIDENCE_RECORDS
        ):
            raise gl.vm.UserError(
        ...
        if (
            max_age < MIN_EVIDENCE_AGE_LIMIT
            or max_age > MAX_EVIDENCE_AGE_LIMIT
        ):
            raise gl.vm.UserError(
        ...
        if (
            min_validity
            < MIN_REMAINING_VALIDITY_LIMIT
            or min_validity
            > MAX_REMAINING_VALIDITY_LIMIT
        ):
            raise gl.vm.UserError(
        ...
        if (
            claim_lifetime < MIN_CLAIM_LIFETIME
            or claim_lifetime > MAX_CLAIM_LIFETIME
        ):
            raise gl.vm.UserError(
        ...
        if (
            content_bytes < MIN_CONTENT_BYTES
        ...
        base_commitment = _keccak_text(
            base_material
        )

        issuer_commitment = _keccak_text(
            "memoryseal-policy-issuers-v1"
            + "\0"
            + policy_id
        )

        origin_commitment = _keccak_text(
        ...
        self.policies[policy_id] = Policy(
            ...
            owner=owner,
            slug=slug,
            version=u64(version_int),
            sealed=False,
            min_evidence_records=u32(min_records),
            min_distinct_issuers=u32(min_issuers),
            min_distinct_origins=u32(min_origins),
            max_evidence_age_seconds=u64(max_age),
            min_remaining_validity_seconds=u64(
                min_validity
            ),
            max_claim_lifetime_seconds=u64(
            ...
            max_evidence_records=u32(max_records),
            max_content_bytes=u32(content_bytes),
            issuer_count=u32(0),
            origin_count=u32(0),
            last_issuer="",
            last_origin="",
        ...
        self.policy_count = u64(
            int(self.policy_count) + 1
        )

        return policy_id

    @gl.public.write
    def add_policy_issuer(
        self,
        policy_id: str,
        issuer_address: str,
    ) -> None:
        ...
        if self.policy_issuer_allowed.get(
            key,
            False,
        ):
            raise gl.vm.UserError(
        ...
        if (
            policy.last_issuer != ""
            and issuer_hex <= policy.last_issuer
        ):
            raise gl.vm.UserError(
        ...
        if int(policy.issuer_count) >= MAX_POLICY_ISSUERS:
            raise gl.vm.UserError(
                "MAX_POLICY_ISSUERS_REACHED"
            )

        policy.issuer_count = u32(
            int(policy.issuer_count) + 1
        )

        policy.last_issuer = issuer_hex

        policy.issuer_commitment = _keccak_text(
    ...
    def add_policy_origin(
        ...
        if self.policy_origin_allowed.get(
            key,
            False,
        ):
            raise gl.vm.UserError(
        ...
        if int(policy.origin_count) >= MAX_POLICY_ORIGINS:
            raise gl.vm.UserError(
                "MAX_POLICY_ORIGINS_REACHED"
            )

        policy.origin_count = u32(
            int(policy.origin_count) + 1
        )

        policy.last_origin = origin

        policy.origin_commitment = _keccak_text(
    ...
    def seal_policy(
        ...
        if (
            int(policy.issuer_count)
            < int(policy.min_distinct_issuers)
        ):
            raise gl.vm.UserError(
        ...
        fingerprint_material = (
            ...
            + policy.origin_commitment
            + "\0"
            + str(int(policy.issuer_count))
            + "\0"
            + str(int(policy.origin_count))
        )

        policy.fingerprint = _keccak_text(
            fingerprint_material
        )
    ...
    def register_evidence(
        ...
        issuer_key = _pair_key(
            policy_id,
            issuer_hex,
        )

        if not self.policy_issuer_allowed.get(
            issuer_key,
            False,
        ):
            raise gl.vm.UserError(
        ...
        origin_key = _pair_key(
            policy_id,
            publisher_origin,
        )

        if not self.policy_origin_allowed.get(
            origin_key,
            False,
        ):
            raise gl.vm.UserError(
        ...
        if latest_version == 0:
            self.lineage_issuers[
                lineage_key
            ] = issuer

            self.lineage_origins[
                lineage_key
            ] = publisher_origin

        else:
            lineage_issuer = self.lineage_issuers[
                lineage_key
            ]
            ...
            lineage_origin = self.lineage_origins[
                lineage_key
            ]
        ...
        self.latest_evidence_versions[
            lineage_key
        ] = u64(version_int)

        self.evidence_count = u64(
            int(self.evidence_count) + 1
        )
    ...
    def propose_claim(
        ...
        return self._propose_claim_internal(
            subject_id,
            claim_text,
            policy_id,
            evidence_ids,
    ...
    @gl.public.write
    def propose_repair_claim(
        self,
        parent_claim_id: str,
        evidence_ids: list[str],
    ) -> str:
        parent = gl.storage.copy_to_memory(
            self._require_claim(
                parent_claim_id
            )
        )
        ...
        if (
            parent.repair_evidence_id
            not in self.evidence_records
        ):
            raise gl.vm.UserError(
                "REPAIR_EVIDENCE_NOT_FOUND"
            )

        offending = gl.storage.copy_to_memory(
            self.evidence_records[
                parent.repair_evidence_id
            ]
        )
        ...
        parent_count = int(
            parent.evidence_count
        )

        if len(evidence_ids) != parent_count:
            raise gl.vm.UserError(
                "REPAIR_EVIDENCE_COUNT_MISMATCH"
            )
        ...
        for index in range(parent_count):
            ...
            parent_stable_ids.append(
                parent_record.stable_record_id
            )

            parent_versions.append(
                int(parent_record.version)
            )
        ...
        for evidence_id in evidence_ids:
            ...
            record = gl.storage.copy_to_memory(
                self.evidence_records[
                    evidence_id
                ]
            )
            ...
            if stable_id in matched_stable_ids:
                raise gl.vm.UserError(
                    "REPAIR_LINEAGE_DUPLICATED"
                )

            matched_stable_ids.append(
                stable_id
            )
        ...
        child_id = self._propose_claim_internal(
            parent.subject_id,
            parent.claim_text,
            parent.policy_id,
            evidence_ids,
            parent.supersedes_claim_id,
            parent_claim_id,
        )

        child = gl.storage.copy_to_memory(
            self._require_claim(
                child_id
            )
        )
    ...
    def _propose_claim_internal(
        ...
        if evidence_count < int(
            policy.min_evidence_records
        ):
            raise gl.vm.UserError(
        ...
        source_set_digest = _keccak_text(
            "memoryseal-source-set-v1"
            + "\0"
            + policy.fingerprint
            + "\0"
            + str(evidence_count)
        ...
        if supersedes_claim_id != "":
            ...
            current_head = (
                self._get_effective_subject_head(
                    subject_id,
                    now,
                )
            )
            ...
            target_policy = self._require_policy(
                target.policy_id
            )

            if (
                target_policy.owner != policy.owner
                or target_policy.slug != policy.slug
            ):
                raise gl.vm.UserError(
                    "SUPERSESSION_POLICY_LINEAGE_MISMATCH"
                )

            if int(policy.version) < int(
                target_policy.version
            ):
                raise gl.vm.UserError(
                    "SUPERSESSION_POLICY_VERSION_REGRESSION"
                )

        claim_hash = _keccak_text(
            "memoryseal-claim-text-v1"
            + "\0"
            + claim_text
        )

        sequence = u64(
            int(self.claim_count) + 1
        )
        ...
        review_deadline = (
            now + MAX_PENDING_CLAIM_SECONDS
        )

        claim_lifetime_deadline = (
            now
            + int(
        ...
        claim = Claim(
            ...
            proposer=proposer,
            evidence_count=u32(evidence_count),
    ...
    def review_claim(
        ...
        claim = gl.storage.copy_to_memory(
        ...
        if claim.supersedes_claim_id == "":
            ...
            target = gl.storage.copy_to_memory(
                self._require_claim(
                    claim.supersedes_claim_id
                )
            )
            ...
            if target.state != CLAIM_SUPPORTED:
                return persist_state(
                    CLAIM_REJECTED,
                    "SUPERSESSION_TARGET_NOT_SUPPORTED",
                )
...
"""
        ...
        if decision == CLAIM_REJECTED:
            if evidence_id != "":
                raise gl.vm.UserError(
                    "UNEXPECTED_REJECTION_EVIDENCE_ID"
                )
        ...
        if (
            ...
            raise gl.vm.UserError(
                "INVALID_SUPPORTED_RESULT"
            )

        valid_until = (
            int(claim.created_at)
            + int(
                policy.max_claim_lifetime_seconds
            )
        )
        ...
        if valid_until <= now:
            return persist_state(
                CLAIM_REPAIR_REQUIRED,
                "NO_CANONICAL_VALIDITY_WINDOW",
            )

        if claim.supersedes_claim_id != "":
            old_head = gl.storage.copy_to_memory(
                self._require_claim(
                    claim.supersedes_claim_id
                )
            )
            ...
            if old_head.state != CLAIM_SUPPORTED:
                raise gl.vm.UserError(
                    "SUPERSESSION_TARGET_STATE_CHANGED"
                )
            ...
            old_head.superseded_by_claim_id = (
                claim_id
            )

            old_head.state_changed_at = (
                now_value
            )
